...
let notificationId = 0; // to uniquely identify each notification
...
const createEmptyWeek = () => {
  ...
  // Calculate this Monday, then add 7 days for NEXT Monday
...
const deleteSplitSchedule = async (slotId: number, day: string) => {
  ...
  // We removed the "todayStr" and "scheduleDayStr" comparison logic here
  ...
  try {
    ...
    if (props.weeklySchedule.schedule && props.weeklySchedule.schedule[day]) {
...
<template>
  ...
  >
    ...
    <div class="relative inline-flex">
        ...
        >
        <button
            class="block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 whitespace-nowrap"
            @click="goThisWeek"
        >
            This Week Schedule
        </button>
      ...
      <div class="flex lg:gap-16 gap-6 place-items-center">
        <div class="flex items-center lg:gap-3 gap-2 relative">
          <!-- Info Icon -->
          <div class="relative group">
            <Info color="green" class="w-5 h-5 cursor-pointer" />

            <!-- Tooltip with smooth animation -->
            <div
              class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-2 text-xs text-white dark:bg-gray-700 bg-[#0078d4] rounded shadow-lg opacity-0 translate-y-2 group-hover:opacity-100 group-hover:-translate-y-1 transition-all duration-300 ease-out pointer-events-none z-10"
            >
              This is based on your submission when you signed up, should you
              need to change this please email <b>mary@ydriveapp.com</b>.
            </div>
          </div>

          <!-- Text -->
          <p
            class="text-gray-400 dark:text-gray-300 text-center text-xs md:text-sm sm:text-xs"
          >
            Required: <b> 0h </b>
          </p>
        </div>
      ...
      >
        ...
        >
          ...
          <div class="flex items-center space-x-3 md:space-x-2 sm:space-x-1">
            <span class="text-sm text-[#0078d4] md:text-[12px] sm:text-[9px] whitespace-nowrap color-blue">
              {{
        ...
        <div
          ...
          :key="schedule.id"
          class="w-full flex justify-between rounded-xl shadow-blue items-center px-2 py-1 cursor-pointer mt-[.5px] border shadow bg-white dark:bg-gray-900 text-[#0078d4] dark:text-white"
        >
