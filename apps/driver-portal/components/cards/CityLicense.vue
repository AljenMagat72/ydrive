...
const { fullName, cityLicenseExp, fetchSecureImage, uploadDocuments, refresh } = useZoho();
...
// Button logic: Disable if currently uploading OR if a valid license is already on file
const isButtonDisabled = computed(() => isUploading.value || (validity.value.isValid && !!cityDocId.value));
...
const handleFileUpload = async (event: Event) => {
  ...
  const renamedFiles = rawFiles.map((file, index) => {
    ...
    const newFileName = `CityLicense_${sanitizedName}_${today}${suffix}.${extension}`;
    return new File([file], newFileName, { type: file.type });
  });

  try {
    // Target the specific TBU field for City License
    await uploadDocuments(renamedFiles, 'TBU_City_License');
    triggerToast('City License uploaded successfully!', 'success');
    ...
    <div class="flex flex-col gap-6 mt-2 flex-grow">
      ...
      <div class="flex flex-col mt-auto">
        ...
        >
          ...
          <template v-else>
            <CheckCircle2 v-if="validity.isValid && cityDocId" class="w-4 h-4 text-blue-500" />
            <Upload v-else class="w-4 h-4" />
            <span>{{ (validity.isValid && cityDocId) ? 'Verified' : 'Upload City License' }}</span>
          </template>
        </button>
