...
import { ShieldCheck, CheckCircle2, AlertCircle, X, Upload } from 'lucide-vue-next';
...
const { fullName, insuranceExp, fetchSecureImage, uploadDocuments, refresh } = useZoho();
...
const insuranceDocId = computed<string | null>(() => {
  const doc = props.details?.Insurance_Photo;
...
const loadImage = async () => {
  ...
  try {
    ...
    const url = await fetchSecureImage(insuranceDocId.value as string);
    if (url) imageBlobUrl.value = url;
  } catch (err) {
    console.error("Insurance Image Load Error:", err);
  } finally {
    imageLoaded.value = true;
  }
};

watch(() => insuranceDocId.value, (newId) => {
  if (newId) loadImage();
...
const validity = computed(() => {
  ...
  if (!expDateStr || expDateStr === '---') {
    return { label: 'Missing', class: 'bg-gray-500/20 text-gray-400 border-gray-500/30', isValid: false };
  }
  ...
  if (diffDays <= 30) return { label: 'Expiring Soon', class: 'bg-yellow-500/20 text-yellow-400 border-red-500/30', isValid: false };
  return { label: 'Valid', class: 'bg-green-500/20 text-green-400 border-green-500/30', isValid: true };
});

const isButtonDisabled = computed(() => isUploading.value || (validity.value.isValid && !!insuranceDocId.value));
...
const handleFileUpload = async (event: Event) => {
  ...
  const renamedFiles = rawFiles.map((file, index) => {
    ...
    const newFileName = `Insurance_${sanitizedName}_${today}${suffix}.${extension}`;
    return new File([file], newFileName, { type: file.type });
  });

  try {
    await uploadDocuments(renamedFiles, 'TBU_Insurance_Policy'); 
    triggerToast('Insurance Policy uploaded successfully!', 'success');
  ...
  <div class="bg-black border border-gray-800 rounded-2xl p-6 flex flex-col gap-4 w-full max-w-sm shadow-blue h-full relative">
    ...
    <div class="flex items-center justify-center w-full gap-2 relative">
      <div class="flex items-center gap-2">
        ...
        <span class="text-lg sm:text-xl font-semibold tracking-tight text-white truncate">Insurance Policy</span>
      </div>
    ...
    <div class="flex flex-col gap-6 mt-2 flex-grow">
      <div class="relative mx-auto w-full sm:w-48 h-32 shrink-0 border-2 border-dashed border-gray-700 rounded-xl flex items-center justify-center bg-gray-900/50 overflow-hidden group">
        <div v-if="insuranceDocId && !imageLoaded" class="absolute inset-0 flex items-center justify-center bg-gray-900/80 z-10">
        ...
        />
        <div v-else-if="!insuranceDocId" class="text-gray-600 text-[10px] text-center uppercase font-bold tracking-widest px-4">
          No Image on File
        </div>
      </div>
      ...
      <div class="flex flex-col justify-top space-y-2 w-full">
        <div class="space-y-1">
          <p class="text-white text-sm flex justify-between gap-2">
            <span class="font-semibold text-gray-400">Policy Expiry:</span> 
      ...
      <div class="flex flex-col mt-auto">
        ...
        >
          ...
          <template v-else>
            <CheckCircle2 v-if="validity.isValid && insuranceDocId" class="w-4 h-4 text-blue-500" />
            <Upload v-else class="w-4 h-4" />
            <span>{{ (validity.isValid && insuranceDocId) ? 'Verified' : 'Upload New Policy' }}</span>
          </template>
        </button>
      ...
      >
        <div v-if="showToast" 
          ...
          :class="toastType === 'success' 
            ? 'bg-green-600/90 border-green-400 text-white shadow-green-900/20' 
            : 'bg-red-600/90 border-red-400 text-white shadow-red-900/20'"
        >
          <div class="bg-white/20 p-1.5 rounded-full shrink-0">
            <CheckCircle2 v-if="toastType === 'success'" class="w-5 h-5 text-white" />
            <AlertCircle v-else class="w-5 h-5 text-white" />
          </div>
          <div class="flex flex-col gap-0.5">
            <span class="text-xs uppercase font-bold tracking-widest opacity-70">
              {{ toastType === 'success' ? 'System Notification' : 'Error Detected' }}
            </span>
