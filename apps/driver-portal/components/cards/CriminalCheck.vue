...
const { fullName, criminalCheckExp, fetchSecureImage, uploadDocuments, refresh } = useZoho();
...
const criminalDocId = computed<string | null>(() => {
  const doc = props.details?.Criminal_Vulnerable;
...
const loadImage = async () => {
  ...
  try {
    ...
    const url = await fetchSecureImage(criminalDocId.value as string);
    if (url) imageBlobUrl.value = url;
  } catch (err) {
    console.error("CERTN Image Load Error:", err);
  } finally {
    imageLoaded.value = true;
  }
};

watch(() => criminalDocId.value, (newId) => {
  if (newId) loadImage();
...
const validity = computed(() => {
  const expDateStr = criminalCheckExp.value;
  ...
  return { label: 'Valid', class: 'bg-green-500/20 text-green-400 border-green-400/30', isValid: true };
});
...
const handleFileUpload = async (event: Event) => {
  ...
  const renamedFiles = rawFiles.map((file, index) => {
    ...
    const newFileName = `VulnerabilityCheck_${sanitizedName}_${today}${suffix}.${extension}`;
    return new File([file], newFileName, { type: file.type });
  }); 

  try {
    await uploadDocuments(renamedFiles, 'TBU_Vulnerability_Check'); 
    triggerToast('Vulnerability Check uploaded successfully!', 'success');
...
<template>
  <div class="dark:bg-black border rounded-2xl p-6 flex flex-col gap-y-4 w-full max-w-sm shadow-blue h-full relative">
    ...
    <div class="flex items-center justify-between w-full gap-2">
      <div class="flex items-center gap-2 min-w-0">
        ...
        <span class="text-lg sm:text-xl font-semibold tracking-tight dark:text-white truncate">Vulnerability Check</span>
      </div>
    ...
    <div class="flex flex-col gap-6 mt-2">
      <div class="relative mx-auto w-full sm:w-48 h-32 shrink-0 border-2 border-dashed border-gray-700 rounded-xl flex items-center justify-center bg-gray-900/50 overflow-hidden group">
        <div v-if="criminalDocId && !imageLoaded" class="absolute inset-0 flex items-center justify-center bg-gray-900/80 z-10">
        ...
        <div v-else-if="!criminalDocId" class="text-gray-600 text-[10px] text-center uppercase font-bold tracking-widest px-4">
          No Image on File
        </div>
      </div>
      ...
      <div class="flex flex-col justify-top space-y-2 w-full flex-grow">
        <div class="space-y-1">
          <p class="text-sm flex justify-between gap-2">
            <span class="font-semibold dark:text-black">Vulnerability Expiry:</span> 
