...
const { criminalCheckExp, fetchSecureImage } = useZoho();
...
const sendEmail = () => {
  ...
  const subject = encodeURIComponent(`CERTN Update Request - ${props.details?.Full_Name || 'Driver'}`);
  let bodyText = `Hello,\n\nI would like to request a CERTN / Criminal Record update.\n\nDriver Name: ${props.details?.Full_Name}\nCurrent Expiration: ${criminalCheckExp.value}\n\n[IMPORTANT]: I have attached my new details to this email.`;
