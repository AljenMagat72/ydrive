...
const { fullName, registrationExp, fetchSecureImage, uploadDocuments, refresh } = useZoho();
...
const isButtonDisabled = computed(() => isUploading.value || (validity.value.isValid && !!ownershipDocId.value));
...
const handleFileUpload = async (event: Event) => {
  ...
  const renamedFiles = rawFiles.map((file, index) => {
    ...
    const newFileName = `Ownership_${sanitizedName}_${today}${suffix}.${extension}`;
    return new File([file], newFileName, { type: file.type });
  });

  try {
    await uploadDocuments(renamedFiles, 'TBU_Ownership'); 
    triggerToast('Ownership document uploaded successfully!', 'success');
    ...
    <div class="flex flex-col gap-6 mt-2 flex-grow">
      ...
      <div class="flex flex-col justify-top space-y-2 w-full">
        <div class="space-y-1">
          <p class="text-white text-sm flex justify-between gap-2">
            <span class="font-semibold text-gray-400">Registration Expiry:</span> 
            <span>{{ registrationExp || '---' }}</span>
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col mt-auto">
        ...
        >
          ...
          <template v-else>
            <CheckCircle2 v-if="validity.isValid && ownershipDocId" class="w-4 h-4 text-blue-500" />
            <Upload v-else class="w-4 h-4" />
            <span>{{ (validity.isValid && ownershipDocId) ? 'Verified' : 'Upload Ownership' }}</span>
          </template>
        </button>
