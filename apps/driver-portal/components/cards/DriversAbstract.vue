...
const { abstractExp, fetchSecureImage } = useZoho();
...
const sendEmail = () => {
  ...
  const subject = encodeURIComponent(`Drivers Abstract Update Request - ${props.details?.Full_Name || 'Driver'}`);
  let bodyText = `Hello,\n\nI would like to request a Drivers Abstract update.\n\nDriver Name: ${props.details?.Full_Name}\nAbstract Expiration: ${abstractExp.value}\n\n[IMPORTANT]: I have attached my new details to this email.`;
      ...
      <div class="flex flex-col justify-top space-y-2 w-full flex-grow">
        <div class="space-y-1">
          <p class="text-white text-sm flex justify-between gap-2">
            <span class="font-semibold text-white">Abstract Expiry:</span> 
            <span>{{ abstractExp || 'N/A' }}</span>
          </p>
        </div>
      </div>
    </div>
