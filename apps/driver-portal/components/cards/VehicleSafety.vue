...
const { fullName, safetyExp, fetchSecureImage, uploadDocuments, refresh } = useZoho();
...
const safetyDocId = computed<string | null>(() => {
  const doc = props.details?.Vehicle_Safety;
...
const loadImage = async () => {
  ...
  try {
    ...
    const url = await fetchSecureImage(safetyDocId.value as string);
    if (url) imageBlobUrl.value = url;
  } catch (err) {
    console.error("Safety Image Load Error:", err);
  } finally {
    imageLoaded.value = true;
  }
};

watch(() => safetyDocId.value, (newId) => {
  if (newId) loadImage();
...
const handleFileUpload = async (event: Event) => {
  ...
  const renamedFiles = rawFiles.map((file, index) => {
    ...
    const newFileName = `Safety_${sanitizedName}_${today}${suffix}.${extension}`;
    return new File([file], newFileName, { type: file.type });
  });

  try {
    await uploadDocuments(renamedFiles, 'TBU_Vehicle_Safety'); 
    triggerToast('Safety document uploaded successfully!', 'success');
  ...
  <div class="bg-black border border-gray-800 rounded-2xl p-6 flex flex-col gap-4 w-full max-w-sm shadow-blue h-full relative">
    ...
    <div class="flex flex-col gap-6 mt-2 flex-grow">
      <div class="relative mx-auto w-full sm:w-48 h-32 shrink-0 border-2 border-dashed border-gray-700 rounded-xl flex items-center justify-center bg-gray-900/50 overflow-hidden group">
        <div v-if="safetyDocId && !imageLoaded" class="absolute inset-0 flex items-center justify-center bg-gray-900/80 z-10">
        ...
        />
        <div v-else-if="!safetyDocId" class="text-gray-600 text-[10px] text-center uppercase font-bold tracking-widest px-4">
          No Image on File
        </div>
      </div>
